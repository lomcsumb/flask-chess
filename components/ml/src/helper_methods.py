import components.ml.src.Bradley as iman
import pandas as pd
...
def init_agent(chess_data, color = 'W'):
    bubs = iman.Bradley(chess_data, color)
    return bubs
...
def agent_vs_agent(bubs, imman):
    ...
    while bubs.game_on():
        ...
        if bubs.get_rl_agent_color() == player_turn:
            ...
            chess_move_str = chess_move_bubs['chess_move_str']
            chess_move_src = chess_move_bubs['move_source']
            print(f'Bubs played {chess_move_str} - from source: {chess_move_src}\n')
            print(bubs.environ.board)
            imman.recv_opp_move(chess_move_bubs)

        # imman's turn
        else:
            chess_move_imman = imman.rl_agent_chess_move()
            chess_move_str = chess_move_imman['chess_move_str']
            chess_move_src = chess_move_imman['move_source']
            print(f'Imman played {chess_move_str} - from source: {chess_move_src}\n')
            print(imman.environ.board)
            bubs.recv_opp_move(chess_move_imman)
